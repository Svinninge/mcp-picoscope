...
def show(label: str, stats: dict) -> None:
    ...
    freq_text = f"{freq:.6g} Hz" if freq else "none"
    ...
    print(
        ...
        f"cycles {stats['cycles_in_record']}"
    )
    if stats["note"]:
...
def main() -> int:
    ...
    try:
        ...
        print(f"\nrange picked: +/-{applied.range_v} V (peak {peak:.4f} V + 20 %)\n")
        ...
        for duration in WINDOWS_S:
            ...
            stats = measure(cap)
            show(f"{duration * 1000:g} ms window", stats)
        ...
        if len(freqs) >= 2:
            ...
            spread = 100 * (hi - lo) / lo
            print(f"\nspread across window lengths: {lo:.6g} .. {hi:.6g} Hz ({spread:.2f} %)")

        cap = scope.capture_block(5 / freqs[0] if freqs else 0.01, 4096)
        cap.capture_id = "signal"
        stats = measure(cap)
        show("5 periods", stats)
        print("image:", export(cap, "png"))
        return 0
