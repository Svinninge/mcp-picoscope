...
**volt/div.** `▼` and `▲` step a 1-2-5 screen scale from 20 mV/div to 5 V/div over eight divisions, and each step is taken on the narrowest device range that covers the screen — 1 V/div (±4 V on screen) uses the ±5 V range. Autoset picks the scale the same way. In auto, the time/div is rounded up to a 1-2-5 step too, so the labels always read as round numbers.
